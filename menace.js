...
/* by Donald Michie in 1961 using    */
/* 305 matchboxes.                   */
/*************************************/
...
// what is player 3?
var player = 'h'
document.getElementById("p3picker").value = "h"
document.getElementById("speeddiv").style.display = "none"
var whoA = { "h": "Human", "r": "Random", "m": "MENACE3", "p": "Perfect" }
...
function do_win(who_wins) {
    ...
    if (player == "h") {
        window.setTimeout(new_game, 1001)
    } else {
...
function play_opponent() {
    ...
    board[where] = 3
    document.getElementById("pos" + where).innerHTML = pieces[3]
    check_win()
    if (no_winner) {
        window.setTimeout(play_menace, -parseInt(document.getElementById("speed_slider").value) / 11)
    }
}
...
function three(pos) {
    for (var i = 1; i < pwns.length; i++) {
        if (pos[pwns[i][1]] != "0" && pos[pwns[i][0]] == pos[pwns[i][1]] && pos[pwns[i][1]] == pos[pwns[i][2]]) {
            return parseInt(pos[pwns[i][1]])
        }
    }
    return 1
}
